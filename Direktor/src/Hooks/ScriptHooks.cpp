...
HOOK_GLOBAL(gml_Script_scr_weapon_variants_init_ultimate, (CInstance* Self, CInstance* Other, RValue* Result, int ArgumentCount, RValue** Arguments) -> RValue*)
{
    ...
    double luckMod = statsObj ? statsObj->InternalReadYYVar(VAR_HASH(mod_luck))->ToDouble() : 1.0;
    ...
    *Self->InternalGetYYVarRef(VAR_HASH(variantInit)) = RValue(true);
    ...
    int32_t rollVariant = variantChoices[(*rng)() % variantChoices.size()];
    // Variant kit 1 = 5 and 16
    // Variant kit 2 = 9 and 12
    // Cursed = 15
    int32_t selectedClass = static_cast<int32_t>(outgameObj->InternalGetYYVarRef(VAR_HASH(selectedclass))->ToDouble());
    int32_t classLevel = static_cast<int32_t>(outgameObj->InternalGetYYVarRef(VAR_HASH(currentclasslvl))->ToRefVector()[3]->ToDouble());
    int32_t selectedSubclass = static_cast<int32_t>(outgameObj->InternalGetYYVarRef(VAR_HASH(selectedsubclass))->ToDouble());
    int32_t subclassLevel = static_cast<int32_t>(outgameObj->InternalGetYYVarRef(VAR_HASH(subclasslvl))->ToRefVector()[3]->ToRefVector()[selectedSubclass]->ToDouble());
    ...
    int32_t tooltipRarity = static_cast<int32_t>(Self->InternalGetYYVarRef(VAR_HASH(tooltip_rarity))->ToDouble());
    ...
    CInstance* pistolExtender = CInstance::FirstOrDefault([&](CInstance* ci) -> bool {
        ...
        if (!ret) return ret;
        if (ci->InternalGetYYVarRef(VAR_HASH(owner))->ToInt32() == Self->InternalGetYYVarRef(VAR_HASH(owner))->ToInt32())
        {
            return true;
        }
        return false;
...
//         Organik::GetLogger()->LogFormatted("Enemy ID: %d, newState: %d", enemyID->ToDouble(), Arguments[1]->ToDouble());
        CInstance* enemyInstance = CInstance::FirstOrDefault([&](CInstance* ci) -> bool {
            return ci && ci->m_ID == static_cast<int32_t>(enemyID->ToDouble());
        });
        if (!enemyInstance)
        {
