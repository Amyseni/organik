...
std::unordered_map<int32_t, PFN_ACTIONHANDLER>* GetActionHandlers()
{
    ...
    {
        actionHandlers = {
            //{ Code_Variable_FindAlloc_Slot_From_Name(nullptr, "apply_variables"), doApplyVariables },
            ...
            //{ Code_Variable_FindAlloc_Slot_From_Name(nullptr, "rng_chance"), doRequiresChance },
...
bool doGetFrom(CInstance* self, CInstance* other, YYObjectBase* context) {
    ...
    if (resultRV && (resultRV->GetKind() == VALUE_REAL || resultRV->GetKind() == VALUE_INT32 || resultRV->GetKind() == VALUE_INT64)) {
        *result = RValue(resultRV->ToDouble() * scaleFactorFinal);
        return true;
    } else {
...
CEvent* CEvent::InsertIntoMap(CHashMap<int64_t, CEvent*, 3>* eventMap, int32_t eventID, int32_t eventSubID,
    PFN_ACTIONHANDLER pfn, const RValue& params, int32_t m_OwnerObjectID, bool appendToChain, bool after)
{
    ...
    Organik::GetLogger()->LogFormatted("CEvent::InsertIntoMap: Retrieved event map element %p", elem);
    if (!elem || !(elem->m_Value)) 
        Error_Show_Action("CEvent::InsertIntoMap: Invalid event map or element!", true, true);
    
    // if (replaceIfExists && elem) {

    //     if (!elem->m_Value->m_Code)
    //         Error_Show_Action("CEvent::InsertIntoMap: Invalid existing event code (null)!", true, true);

    //     elem->m_Value->m_Code->m_Functions->m_CodeFunction = pfn;
    //     elem->m_Value->m_Code->m_Functions->m_Name = "EVENT:HACKED";
    //     elem->m_Value->m_Code->m_Name = "EVENT:HACKED";
    //     elem->m_Value->m_Code->m_Prototype = params.ToObject();
        
    //     elem->m_Value->m_OwnerObjectID = m_OwnerObjectID;
    ...
    if (elem && elem->m_Value) {
        ...
        if (pAction->m_Token.m_Value.GetKind() != VALUE_ARRAY || pAction->m_Token.m_ArrLength < 2)
        {
            ...
            Organik::GetLogger()->LogFormatted("CEvent::InsertIntoMap: Created outer array with 2 inner arrays");
            ...
            refPair1.SetPairSecond(after ? nullptr : parameters);
            refPair2.SetPairFirst(after ? pfn : original);
            refPair2.SetPairSecond(after ? parameters : nullptr);
        ...
        if (!appendToChain)
        {
            ...
            if(after)
            {
                ...
                //? yay the easy one
                pAction->m_Token.m_Value.push_back(RValue(std::vector{newActionPair}));
            ...
            else
            {
                ...
                pAction->m_Token.m_Value[targetPosition] = RValue(std::vector{newActionPair});
        ...
        else
        {
            ...
            pAction->m_Token.m_Value[targetPos].push_back(newActionPair);
            return elem->m_Value;
        }
    }
...
//     return true;
// }
// bool doApplyVariables(CInstance* self, CInstance* other)
// {
...
//     return true;
// }
// bool doCallScript(CInstance* self, CInstance* other)
// {
