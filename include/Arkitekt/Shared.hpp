...
bool EventHooks::## _ ## _obj ## Hook ## _event ## _subevent ## _priority ## _id ::Install() { \
	GetLogger()->LogFormatted("EventHooks::_" ## #_obj ## "Hook" ## #_event ## #_subevent ## #_priority ## #_id ## "::Install %p\n",reinterpret_cast<void*>(this)); \
	if (!this->m_HookObj){ \
		this->m_HookObj = new FunctionHook( \
			this->functionName, \
			&this->m_FnDef, \
...
void EventHooks::## _ ## _obj ## Hook ## _event ## _subevent ## _priority ## _id ::Uninstall() { \
	GetLogger()->LogFormatted("EventHooks::_" ## #_obj ## "Hook" ## #_event ## #_subevent ## #_priority ## #_id ## \
		"::Uninstall %p\n",reinterpret_cast<void*>(this)); \
	if (this->m_HookObj) \
	{ \
		GetLogger()->LogFormatted("EventHooks::_" ## #_obj ## "Hook" ## #_event ## #_subevent ## #_priority ## #_id ## \
...
void EventHooks::## _ ## _obj ## Hook ## _event ## _subevent ## _priority ## _id ::Init() { \
	GetLogger()->LogFormatted("EventHooks::_" ## #_obj ## "Hook" ## #_event ## #_subevent ## #_priority ## #_id ## "::%s\n", "Init"); \
...
static auto ___ ## _ ## _obj ## Hook ## _event ## _subevent ## _priority ## _id ## ___INITIALIZER = []() -> auto { \
	auto ret = std::thread([]() -> auto { \
		...
		while (!ARKDone()) { \
			std::this_thread::sleep_for(std::chrono::milliseconds(25)); \
			if (++retryCount > 5000) { \
				GetLogger()->LogFormatted(#_obj ## "Hook" ## #_event ## #_subevent ## #_priority ## #_id ## \
					": %s", "Timed out waiting for ARK Init to finish"\
				); \
				break; \
			} \
			continue; \
...
bool ScriptHooks::## _ ## _script ## Hook ## _priority ## _id ::Install() { \
	GetLogger()->LogFormatted("ScriptHooks::_" ## #_script ## "Hook" ## #_priority ## #_id ## "::Install %p\n ",reinterpret_cast<void*>(this)); \
	if (!this->m_HookObj){ \
		this->m_HookObj = new FunctionHook( \
			this->functionName, \
			&this->m_FnDef, \
...
void ScriptHooks::## _ ## _script ## Hook ## _priority ## _id ::Uninstall() { \
	GetLogger()->LogFormatted("ScriptHooks::_" ## #_script ## "Hook" ## #_priority ## #_id ## \
		"::Uninstall %p\n",reinterpret_cast<void*>(this)); \
	if (this->m_HookObj) \
	{ \
		GetLogger()->LogFormatted("ScriptHooks::_" ## #_script ## "Hook" ## #_priority ## #_id ## \
...
void ScriptHooks::## _ ## _script ## Hook ## _priority ## _id ::Init() { \
	GetLogger()->LogFormatted("ScriptHooks::_" ## #_script ## "Hook" ## #_priority ## #_id ## "::%s\n", "Init"); \
	int scriptID = Script_Find_Id(#_script) - 100000; \
	GetLogger()->LogFormatted("ScriptHooks:Add - Script ID %d", scriptID); \
	CScript* theScript = ScriptFromId(scriptID); \
	if (!theScript)  \
		Error_Show_Action_Alt(true, true, "ScriptHooks::Add: Invalid script or index. Found ID %d", scriptID); \
	ScriptHooks::## _ ## _script ## Hook ## _priority ## _id ::s_Script = theScript; \
...
static auto ___ ## _ ## _script ## Hook ## _priority ## _id ## ___INITIALIZER = []() -> auto { \
	auto ret = std::thread([]() -> auto { \
		...
		while (!ARKDone()) { \
			std::this_thread::sleep_for(std::chrono::milliseconds(25)); \
			if (++retryCount > 5000) { \
				GetLogger()->LogFormatted(#_script ## "Hook" ## #_priority ## #_id ## \
					": %s", "Timed out waiting for ARK Init to finish"\
				); \
				break; \
			} \
