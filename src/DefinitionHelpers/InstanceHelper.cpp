...
void Organik::DoHelperSetup()
{
    ...
    for (int i=0;i<variableCount;i++)
    {
        ...
        if (hash)
        {
            DO_LOG2("adding %d to variable hashes", hash);
    ...
    for (int i = Objects::obj_reserved0; i <= Objects::__YYInternalObject__2496; i++)
    {
    ...
    {
        RValue r = DoBuiltin(&gml_asset_get_index, Organik::Sounds::SoundNamesArr[i] );
    ...
    {
        RValue r = DoBuiltin(&gml_asset_get_index,  Organik::Rooms::RoomNamesArr[i]).ToInt32();
