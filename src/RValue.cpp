...
uint32_t RValue::GetKind(const RValue& rv) noexcept
{
	return rv.GetKind( );
}

uint32_t RValue::GetKind( ) noexcept
{
	switch (this->m_Kind & MASK_KIND_RVALUE)
	{
	case 0:
...
uint32_t RValue::GetKind( ) const noexcept
{
	switch (this->m_Kind & MASK_KIND_RVALUE)
	{
	case 0:
...
// 	}
// 	Error_Show_Action(
// 		GetLogger()->ParseFormatting("Expected MAP, got %s", GetKindName()).data(),
// 		true, true
// 	);
// 	return nullptr;
// }
...
// 	}
// 	Error_Show_Action(
// 		GetLogger()->ParseFormatting("Expected OBJECT, got %s", GetKindName()).data(),
// 		true, true
// 	);
// 	return nullptr;
// }
...
{
	if ((m_Kind & MASK_KIND_RVALUE) == (VALUE_OBJECT & MASK_KIND_RVALUE))
	{
		...
		return (
			pObj->InternalReadYYVar(
				Code_Variable_FindAlloc_Slot_From_Name(
					pObj,
					const_cast<char*>(_name.data( )))) != nullptr);
	}
	else if ((m_Kind & MASK_KIND_RVALUE) == (VALUE_ARRAY & MASK_KIND_RVALUE))
	{
...
bool RValue::ContainsValue(int32_t idx) const
{
	if ((m_Kind & MASK_KIND_RVALUE) == (VALUE_OBJECT & MASK_KIND_RVALUE))
	{
		...
		return (
			pObj->InternalReadYYVar(idx)) != nullptr;
	}
	else if ((m_Kind & MASK_KIND_RVALUE) == (VALUE_ARRAY & MASK_KIND_RVALUE))
	{
...
{
	auto value = round(*reinterpret_cast<const double*>(std::launder(&this->m_Real)));
	value = abs(static_cast<int32_t>(value));
	return (value >= 1);
}
...
{
	if ((this->m_Kind & MASK_KIND_RVALUE) != (VALUE_ARRAY & MASK_KIND_RVALUE))
		return {};
...
int32_t RValue::GetArrayLength( ) const
{
	...
	if ((m_Kind & MASK_KIND_RVALUE) != (VALUE_ARRAY & MASK_KIND_RVALUE))
	{
		Error_Show_Action(
			GetLogger( )->ParseFormatting("RValue::GetArrayLength: Expected VALUE_ARRAY, got %s", GetKindName( )).data( ),
			true, true);
		return 0;
	}
...
RValue* RValue::ToArray( )
{
	return &(*reinterpret_cast<RValue**>(
		(&reinterpret_cast<char*>(*std::launder(&this->m_Pointer))[ARRAY_OFFSET]))[0]);
}
...
RValue& RValue::push_back(const RValue& value)
{
	...
	{
		Error_Show_Action(
			GetLogger( )->ParseFormatting("RValue::push_back: Expected VALUE_ARRAY, got %s", GetKindName( )).data( ),
			true, true);
		return *this;
	}
	RefDynamicArrayOfRValue* arrayRef =
	...
	arrayRef->m_Length += 1;
	YYRealloc(&(void*&) arrayRef->m_Array, (arrayRef->m_Length) << 4);
	...
	*reinterpret_cast<RValue*>(std::launder(this)) = RValue( );
	...
	for (size_t index = 0; index < Values.size( ); index++)
	{
		...
		RValue* member_value = &(*reinterpret_cast<RValue**>(
			&reinterpret_cast<char*>(*std::launder(&this->m_Pointer))[ARRAY_OFFSET])[index]);
...
RValue::RValue(bool Value)
{
	...
	*reinterpret_cast<double*>(std::launder(&this->m_Real)) = Value ? 1.0 : 0.0;
...
RValue& RValue::back( )
{
	if ((this->m_Kind & MASK_KIND_RVALUE) != (VALUE_ARRAY & MASK_KIND_RVALUE))
		Error_Show_Action(
			GetLogger( )->ParseFormatting("RValue::back: Expected VALUE_ARRAY, got %s with flag %d", GetKindName( ), m_Flags).data( ),
			true, true);
...
RValue& RValue::operator[](size_t Index)
{
	...
	if (!((this->m_Kind & MASK_KIND_RVALUE) == (VALUE_ARRAY & MASK_KIND_RVALUE)))
		Error_Show_Action(
			GetLogger( )->ParseFormatting("RValue::operator[]: Expected VALUE_ARRAY, got %s with flag %d", GetKindName( ), m_Flags).data( ),
			true, true);
	...
	if (Index < 0 || Index >= static_cast<size_t>(arrayRef->m_Length))
		Error_Show_Action(
			GetLogger( )->ParseFormatting("RValue::operator[]: Index %zu out of bounds for array of length %d", Index, arrayRef->m_Length).data( ),
			true, true);
...
RValue& RValue::operator[](RValue Index)
{
	...
	{
		case constexpr (VALUE_ARRAY & MASK_KIND_RVALUE) :
			return ((*this)[(idx)]);
			case constexpr (VALUE_OBJECT & MASK_KIND_RVALUE) :
				return *this->ToObject( )->InternalGetYYVarRef(int32_t(idx));
		default:
			Error_Show_Action(
				GetLogger( )->ParseFormatting("RValue::operator[]: Expected VALUE_ARRAY or VALUE_OBJECT, got %s with flag %d", GetKindName( ), m_Flags).data( ),
				true, true);
			break;
	}
	return *this;
...
{
	int32_t hash = 0;
	if (index.GetKind( ) == (VALUE_STRING & MASK_KIND_RVALUE))
	{
		hash = Code_Variable_FindAlloc_Slot_From_Name(nullptr, index.ToCString( ));
	}
	else if (index.GetKind( ) == (VALUE_INT32 & MASK_KIND_RVALUE) || index.GetKind( ) == (VALUE_REAL & MASK_KIND_RVALUE))
	{
	...
	{
		Error_Show_Action(
			GetLogger( )->ParseFormatting("ReadStructValue: Expected string or int32 for member name, got %s", index.GetKindName( )).data( ),
			true, true);
		return *p_Struct;
	}
	if ((p_Struct->m_Kind & MASK_KIND_RVALUE) != (VALUE_OBJECT & MASK_KIND_RVALUE))
	{
		Error_Show_Action(
			GetLogger( )->ParseFormatting("ReadStructValue: Expected VALUE_OBJECT, got %s", p_Struct->GetKindName( )).data( ),
			true, true);
		return *p_Struct;
	}
	...
	{
		Error_Show_Action(
			GetLogger( )->ParseFormatting("ReadStructValue: Object does not contain member with hash 0x%X", hash).data( ),
			true, true);
		return *p_Struct;
	}
	return *pVal;
