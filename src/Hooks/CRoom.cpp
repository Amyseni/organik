...
_NODISCARD_LOCK std::pair<std::unique_lock<std::shared_mutex>, std::unordered_map<int32_t, CInstance*>> GetActiveInstances()
{
    ...
    if (*Run_Room)
    {
        for (i = (*Run_Room)->m_Active.m_First; i; i = i->m_Next)
        {
            if (!i)
                continue;
...
HOOK_METHOD(CRoom, UpdateActive, (void) -> void)
{
    ...
    if (!Organik::Utils::isInitializationDone())
        return;
    DO_LOG1("Updated active instances in the room. Total count: %d", m_Active.m_Count);
}
