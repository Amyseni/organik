...
std::mt19937 gen(std::clock()); // Standard mersenne_twister_engine
...
std::pair<std::unique_lock<std::shared_mutex>, Utils::Settings::GUISettings*&> Utils::Settings::GetUISettings()
{
    ...
    return std::pair<std::unique_lock<std::shared_mutex>, Utils::Settings::GUISettings*&>(std::move(lock), std::forward<Utils::Settings::GUISettings*&>(g_UISettings));
        ...
        if (hConnect != NULL) 
        {
            ...
            {   
                ...
                if (isSent)
                {
                    ...
                    {
                        DO_LOG0("HTTP Status: %s", statusCode);
                    }
                    ...
                    DO_LOG0("Response length: %d", responseStr.length());
                }
                else
                {
                    DWORD error = GetLastError();
                    DO_LOG0("HttpSendRequestA failed with error: %d", error);
                }
...
{
    std::string_view playerName = "Unknown Player";
    if ((ImGui::GetCurrentContext())) //this means engine init has finished, safe to look at outgame now
    {
        
    }
    std::string_view formattedErrorMsg = GetLogger()->ParseFormatting("Bug Report - %s\n\n%s", playerName.data(), errorMsg.data());
    std::cerr << GetLogger()->ParseFormatting("Report Message: %s -> %s", errorMsg.c_str(), formattedErrorMsg.data());
    std::string_view body = "content=" + Utils::url_encode(formattedErrorMsg.data());
    std::cerr << GetLogger()->ParseFormatting("Encoded: %s", body.data());
    ...
    std::cerr << GetLogger()->ParseFormatting("Response: %s", response.data());
}
__declspec(dllexport) void __cdecl Error_Show_Action(const char* err, bool serious, bool _manualError);
#define DO_WEBHOOK false

HOOK_GLOBAL(Error_Show_Action, (char* message, bool mustCrash, bool manualError) -> void)
{
    std::string_view formattedMessage = GetLogger()->ParseFormatting("Oops! All Exceptions!\nMustCrash? %s\nManualError? %s\nMessage: %s",
        mustCrash ? "yes" : "no",
        manualError ? "yes" : "no",
        message
    );
    ...
    MessageBoxA(NULL, formattedMessage.data(), manualError ? "Organik Error" : "Synthetik Error", MB_ICONERROR | MB_OK);
    if (mustCrash)
    {
