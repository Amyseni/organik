...
void DrawObjectSelector(std::function<void(int32_t)> callback)
{
    ...
    {
        for (uint32_t i = Organik::Objects::obj_null; i <= Organik::Objects::__YYInternalObject__2496; i++)
        {

            const char* objName = Organik::Objects::ObjectNamesArr[i];
            const int32_t objIndex = i;
...
void Variant::Apply(CInstance* instance, int variant, int sClass, int subclass, int subclassLevel)
{
    ...
    if (!instance)
   	return;
        CInstance* statsObj = CInstance::FirstOrDefault([](CInstance* ci) -> bool {
            return ci && ci->m_ObjectIndex == Organik::Objects::obj_stats;
    });
    if (!statsObj)
        return;
    ...
    if ((weaponvarianticon->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNDEFINED & MASK_KIND_RVALUE))
    ...
    if ((weaponvarianticoncolor->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNDEFINED & MASK_KIND_RVALUE) || ((weaponvarianticoncolor->GetKind() & MASK_KIND_RVALUE)& MASK_KIND_RVALUE)== (VALUE_UNSET & MASK_KIND_RVALUE))
        *weaponvarianticoncolor = RValue(Rarities::RarityColor[Rarities::COMMON]);
    if ((variantsprite->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNDEFINED & MASK_KIND_RVALUE) || (variantsprite->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNSET & MASK_KIND_RVALUE))
        *variantsprite = RValue(-1.0);
    if ((variantcolor->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNDEFINED & MASK_KIND_RVALUE) || (variantcolor->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNSET & MASK_KIND_RVALUE))
        *variantcolor = RValue(RValue(Rarities::RarityColor[Rarities::COMMON]));
    if ((weaponvariantname->GetKind() & MASK_KIND_RVALUE) == (VALUE_UNDEFINED & MASK_KIND_RVALUE) || (weaponvariantname->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNSET & MASK_KIND_RVALUE))
        YYCreateString(weaponvariantname, "NAME"); ;
    if ((weaponvariantnameshort->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNDEFINED & MASK_KIND_RVALUE) || (weaponvariantnameshort->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNSET & MASK_KIND_RVALUE))
        YYCreateString(weaponvariantnameshort, "GAY"); 
    if ((variantmanufacturer->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNDEFINED & MASK_KIND_RVALUE) || (variantmanufacturer->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNSET & MASK_KIND_RVALUE))
        YYCreateString(variantmanufacturer, "ORGANIK"); 
    if ((varianttext->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNDEFINED & MASK_KIND_RVALUE) || (varianttext->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNSET & MASK_KIND_RVALUE))
        YYCreateString(varianttext, "PLACEHOLDER");
    if ((varianttextnegative->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNDEFINED & MASK_KIND_RVALUE) || (varianttextnegative->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNSET & MASK_KIND_RVALUE))
        YYCreateString(varianttextnegative, "PLACEHOLDER");
    if ((mod_magazinebonus->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNDEFINED & MASK_KIND_RVALUE) || (mod_magazinebonus->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNSET & MASK_KIND_RVALUE))
        *mod_magazinebonus = RValue(0.01);
    if ((mod_magazinesize->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNDEFINED & MASK_KIND_RVALUE) || (mod_magazinesize->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNSET & MASK_KIND_RVALUE))
        *mod_magazinesize = RValue(1.0);
    if ((default_magsize_rounding->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNDEFINED & MASK_KIND_RVALUE) || (default_magsize_rounding->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNSET & MASK_KIND_RVALUE))
        *default_magsize_rounding = RValue(1.0);
    if ((ammo_max->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNDEFINED & MASK_KIND_RVALUE) || (ammo_max->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNSET & MASK_KIND_RVALUE))
        *ammo_max = RValue(60.0);
    if ((screenshakeamount->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNDEFINED & MASK_KIND_RVALUE) || (screenshakeamount->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNSET & MASK_KIND_RVALUE))
        *screenshakeamount = RValue(0.0);
    if ((tooltip_rarity->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNDEFINED & MASK_KIND_RVALUE) || (tooltip_rarity->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNSET & MASK_KIND_RVALUE))
        *tooltip_rarity = RValue(1.0);
    if ((rollvariant->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNDEFINED & MASK_KIND_RVALUE) || (rollvariant->GetKind() & MASK_KIND_RVALUE)== (VALUE_UNSET & MASK_KIND_RVALUE))
    ...
    *screenshakeamount = RValue(0.0);
    ...
    if (getUserVariants()->contains(variant))
    {
        ...
        *variantsprite = RValue(static_cast<double>(userVariant.spriteID));
        ...
        {
            DoBuiltin(&gml_audio_play_sound, soundID, 15.0, false);
        }
    }
    ...
    for (const auto& [statHash, increase] : statIncreases)
    {
        ...
        if (statVar)
        {
            *statVar = RValue(parseRValueNumber<double>(statVar) + increase);
    ...
    for (const auto& [statHash, multiplier] : statMultipliers)
    {
        ...
        if (statVar)
        {
            *statVar = RValue(parseRValueNumber<double>(statVar) * multiplier);
    ...
    double ammoMaxVal = parseRValueNumber<double>(ammo_max) ? parseRValueNumber<double>(ammo_max) : 1.0;
    double defaultMagSizeRoundingVal = parseRValueNumber<double>(default_magsize_rounding) ? parseRValueNumber<double>(default_magsize_rounding) : 1.0;
    double modMagBonus = parseRValueNumber<double>(mod_magazinebonus) ? parseRValueNumber<double>(mod_magazinebonus) : 0.0;
    double modMagSize = parseRValueNumber<double>(mod_magazinesize) ? parseRValueNumber<double>(mod_magazinesize) : 1.0;
    if (modMagBonus < 0.0)
        modMagBonus = 0.01;
    if (modMagSize < 1.0)
        modMagSize  = 1.01;
