...
void ParseJSONRValue(RValue* jsonValue, RValue* out) {
    ...
    Log("ParseJSONRValue: Parsing RValue of kind %s", jsonValue->GetKindName());
    ...
    else if (kind == VALUE_ARRAY)
    {
        ...
        *out = RValue();
        ...
        {
            Log("ParseJSONRValue: Parsing array item of type: %s", item->GetKindName());

            RValue parsedValue(5);
            ParseJSONRValue(item, &parsedValue);
            out->push_back(parsedValue);
            Log("ParseJSONRValue: added item of type: %s to vector", out->back().GetKindName());
        }
    ...
    else if (kind == VALUE_OBJECT)
    {
        ...
        for (auto name : refVec)
        {
            ...
            Log("ParseJSONRValue: Parsing object member: %s (%d)", 
            ...
            Log("ParseJSONRValue: added item of type: %s to map", 
                (*pObj)[propHash]->GetKindName());
        }
    }
    else if (kind == VALUE_BOOL)
    {
        Log("ParseJSONRValue: Parsing boolean value: %s", 
        ...
        *out = RValue(jsonValue->ToBoolean());
        ...
        Log("ParseJSONRValue: Parsed boolean RValue of kind %s", (*out).GetKindName());
    }
    // Integer handling
    ...
    Log("ParseJSONRValue: Parsed RValue of kind %s", (*out).GetKindName());
}
...
//         });
//         if (!targetInstance) {
//             Log("doRequiresVars: No instance found with object index %d", objectHash);
//             *result = RValue(false);
//             return false;
//         }
...
//                 Log("doRequiresVars: 'equal' currently unimplemented");
//                 return tmp != nullptr;
...
//                 return tmp == nullptr;
//             } else {
//                 Log("doRequiresVars: Unknown mode hash %d", modeHash);
//                 *result = RValue(true);
//                 return true;
//             }
//         }
...
std::unordered_map<int32_t, PFN_ACTIONHANDLER>* GetActionHandlers()
{
    ...
    {
        actionHandlers = {
            { VAR_HASH(apply_variables), nullptr },
            ...
            { VAR_HASH(rng_chance), nullptr },
            { VAR_HASH(variable_get), nullptr },
        };
    }
...
bool doGetFrom(CInstance* self, CInstance* other, YYObjectBase* context) {
    ...
    Log("doGetFrom: Starting with optimized flag: %s", 
        optimized->ToBoolean() ? "true" : "false");
    if (!optimized->ToBoolean() || true ) {
        Log("doGetFrom: Processing 'from' parameter");
        ...
        Log("doGetFrom: Parsed 'from' as object '%s' and variable '%s'", 
        ...
        Log("doGetFrom: Resolved object '%s' to hash %d", objectPart.c_str(), objectHash);
        ...
        Log("doGetFrom: Parsed 'from' as object hash %d and variable hash %d", objectHash, variableHash);
    ...
    Log("doGetFrom: Retrieving values using object hash and variable hash");
    ...
    Log("doGetFrom: Looking for instance. %08x", objectHash);
    ...
    {
        Log("doGetFrom: 'from' resolves to self instance");
        targetInstance = self;
    }
    else if (objectHash == VAR_HASH(other) && other)
    {
        Log("doGetFrom: 'from' resolves to other instance");
        targetInstance = other;
    }
    else {
        Log("doGetFrom: Looking for instance with object hash %d", objectHash);
    ...
    Log("doGetFrom: Found instance %p", targetInstance);
    ...
    if (!scaleRV) {
        Log("doGetFrom: No scale parameter, returning raw value of kind %s", resultRV->GetKindName());
        *result = resultRV ? *resultRV : RValue(1.0);
        return true;
    }
    
    double scaleFactorFinal = 1.0;
    Log("doGetFrom: Processing scale factor of kind %s", scaleRV->GetKindName());
    ...
    Log("doGetFrom: Applying scale factor %f", scaleFactorFinal);
    ...
    Log("doGetFrom: Warning: reached end of function unexpectedly");
    *result = RValue(1.0);
    return true;
}
...
// {
//     Log("ParseActions: Starting to parse actions for object %d, event %d/%d", 
//         objectIndex, eventCode, eventSubCode);

//     if (!jsonValue) {
//         Log("ParseActions: No JSON value provided");
//         return false;
//     }
...
//     Log("ParseActions: Parsing requires actions array of size %d from %p", (jsonValue->InternalReadYYVar(VAR_HASH(action_requires))->GetArrayLength()), jsonValue);
...
//             Log("ParseActions: Failed to process action item in requires array");
...
//     Log("ParseActions: Parsing actions array of size %d from %p", (jsonValue->InternalReadYYVar(VAR_HASH(actions))->GetArrayLength()), jsonValue);
...
//             Log("ParseActions: Failed to process action item in actions array");
//         ClearPatchCache();
//     }

//     return true;
// }
//         Log("ParseTriggers: After parsing, trigger %d has kind %s (flags=%d)", 
//             triggerIndex-1, objParsed.GetKindName(), objParsed.m_Flags);
        
//         if (objParsed.m_Kind != VALUE_OBJECT) {
//             Log("ParseTriggers: Skipping parsed non-object trigger %d", triggerIndex-1);
//             continue;
//         }
...
//                 Log("ParseTriggers: Trigger %d is an event trigger", triggerIndex-1);
//             }
//             else 
//             {
...
//             Log("ParseTriggers: Parsed member: Key hash %d, Type: %s", 
...
//         Log("ParseTriggers: Trigger %d map at %p with %d members", triggerIndex-1, obj, obj->m_YYVarsMap->m_CurrentSize);
        
//         // Log all keys in the map
//         Log("ParseTriggers: Trigger %d keys:", triggerIndex-1);
//         for (const auto& [key, val] : *obj) {
//             Log("  - Key hash: %d, Type: %s", key, val.GetKindName());
//         }

//         RValue typeRV = (*obj)[VAR_HASH(type)];
//         Log("ParseTriggers: Trigger %d type RValue is kind %s", 
...
//         );
//         Log("ParseTriggers: Trigger %d type hash = %d", triggerIndex-1, type);
...
//             Log("ParseTriggers: 'actions' field is %d", 
...
//             Log("ParseTriggers: 'action_requires' field is %d",
...
//             Log("ParseTriggers: 'subevent' field is %d",
...
//             Log("ParseTriggers: Event name is '%s'", eventName.c_str());

//             if (!eventName.length()) {
//                 Log("ParseTriggers: ERROR - Event name is empty, skipping");
...
//             Log("ParseTriggers: Object index resolved to %d", objIndex);
            
//             if (objIndex <= 0) {
//                 Log("ParseTriggers: ERROR - Invalid object index %d (%d, %s), breaking", 
...
//                 Log("ParseTriggers: Object is 'global (outgame)' (index %d)", objIndex);
//             }
...
//             Log("ParseTriggers: Requires array at %p with %zu entries", 
//                 requiresArray, requiresArray.size());

//             // Log all flags
//             Log("ParseTriggers: Requires array contains:");
//             for (auto& condition : requiresArray) {
//                 Log("  - Type: %s, Flags: %d",
//                     condition->GetKindName(), condition->m_Flags);

//                 if (condition->GetKind() != VALUE_MAP) {
//                     Log("ParseTriggers: 'requires' condition is of kind %s, expected object", condition->GetKindName());
...
//                 Log("ParseTriggers: Processing 'requires' condition map at %p with %d entries", 
...
//                     Log("ParseTriggers: 'requires' condition object index defaulted to trigger owner %d", trigger->objectOwner);
//                 }
...
//                     break;
//                 }
//                 Log("ParseTriggers: 'requires' condition type is %d", condType);
//                 DoAction* condFnc;
...
//                     Log("ParseTriggers: Creating new requiresAction for condition of type %d", condType);
//                     requiresAction = Action::Create(*condFnc, map);
//                     requiresAction->type = condType;
//                 }
//                 else {
//                     Log("ParseTriggers: Appending condition of type %d to requiresAction at %p", condType, requiresAction);
...
{
    Log("doGetFrom: Checking if recursion is neeeded for context %p", context);
    RValue *fromStr = context->InternalGetYYVarRef(VAR_HASH(from));
...
bool doApplyVariables(CInstance* self, CInstance* other, YYObjectBase* context)
{
    ...
    if (optimizationRV->m_Kind != VALUE_BOOL)
    {
        ...
        *optimizedVarAlloc = RValue(true);
        ...
        Log("doApplyVariables: scaleVar is %p of kind %s", scaleVar, scaleVar ? scaleVar->GetKindName() : "null");
        auto objectVar = context->InternalGetYYVarRef(VAR_HASH(object));
        Log("doApplyVariables: objectVar is %p of kind %s", objectVar, objectVar ? objectVar->GetKindName() : "null");
        ...
        {
            Error_Show_Action_Alt(true, true, "doApplyVariables: 'object' parameter is of invalid type %s.", objectVar->GetKindName());
        }

        if (objectVar->GetKind() == VALUE_STRING)
        {
            Log("doApplyVariables: resolving object name '%s' to index", objectVar->ToCString());
        ...
        {
            Log("doApplyVariables: resolving object index from value of kind %s", objectVar->GetKindName());
        ...
        Log("doApplyVariables: objectVar is now %d", parseRValueNumber<int32_t>(objectVar));
        if (scaleVar)
        {
            Log("doApplyVariables: optimizing scaleVar %p of kind %s", scaleVar, scaleVar->GetKindName());
        ...
        {
            Log("doApplyVariables: optimizing variable entry key %d of kind %s",
                elem->m_Key, elem->m_Value ? elem->m_Value->GetKindName() : "null");
            if (elem->m_Value->GetKind() == VALUE_OBJECT)
            {
                Log("doApplyVariables:  - is object, optimizing");
    ...
    Log("doApplyVariables: target instance is %p", targetInstance);
    if (!targetInstance)
    {
    ...
    Log("doApplyVariables: scale multiplier is %f", multiplier);
    ...
    if (varsToApplyObj && varsToApplyObj->GetKind() == VALUE_OBJECT)
    {
        ...
        {
            if (!elem->m_Value) Log("doApplyVariables: found null value for key %d, hash %d, hash_deleted %s",
