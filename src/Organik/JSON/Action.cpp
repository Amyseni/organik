...
std::unordered_map<int32_t, PFN_ACTIONHANDLER>* GetActionHandlers()
{
    ...
    {
        actionHandlers = {
            { VAR_HASH(apply_variables), nullptr }, // &doApplyVariables },
            ...
            { VAR_HASH(rng_chance), nullptr }, //&doRequiresChance },
            { VAR_HASH(variable_get), nullptr }, // &doGetFrom },
        };
    }
    ...
    PFN_ACTIONHANDLER pfn, YYObjectBase* actionActualInnerParams, int32_t m_OwnerObjectID, bool appendToChain, bool after)
{
    ...
    if (after)
    {
        ...
        MM_ReAlloc(&(void*&) (elem->m_Value->m_Code->m_Token.m_AfterArray), len << 3, __FILE__, 0x4a7);
    ...
    else
    {
        ...
        MM_ReAlloc(&(void*&) (elem->m_Value->m_Code->m_Token.m_BeforeArray), len << 3, __FILE__, 0x4a7);
