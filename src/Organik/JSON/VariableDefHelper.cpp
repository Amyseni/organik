...
bool VariableDefHelper::Load() 
{
    ...
    for (auto elem : *(defObj->m_YYVarsMap))
    {
...
void VariableDef::DrawDisplayValueInTable(RValue* value){
    ...
    ImGui::Text("%s", GetRepresentation(*value).data());
}
...
const std::string_view& VariableDef::GetRepresentation(const RValue& value) const
{
    ...
    case TYPE_STRING:
        ...
        break;
    case TYPE_COLOR:
        return std::string_view(GetLogger()->ParseFormatting("#%06X",
    ...
    case TYPE_BOOL:
        ...
        break;
    default:
        return std::string_view(GetLogger()->ParseFormatting("%.2f",
            parseRValueNumber<double>(value)
        ).data());
    }
}
