...
#define OBJ_INDEX(obj) Organik::Objects::ObjIndexes[Organik::Objects:: obj]
    ...
    if (ImGui::BeginMenu("User Settings"))
    {
        out_mousedOver = true;
            ...
            if (ImGui::MenuItem("Roll Me (Randomize Class)", "Ctrl+Shift+I", Settings::g_EnableInvincibility, !gamestarted))
            {
                ...
                if (classPerks.size() > 0)
                {
                    ...
                    for (uint i=0; i<perkGroups.size(); i++)
                    {
                        ...
                        if (parseRValueNumber<double>(perkGroup) == 2.0) {
                            pistolPerks.push_back(RValue(static_cast<double>(i)));
                            continue;
                        }

                        if (parseRValueNumber<double>(perkGroup) == 4.0) {
                            corePerks.push_back(RValue(static_cast<double>(i)));
                            continue;
                        }
                        double classId = parseRValueNumber<double>(perkClasses[i]);
                    ...
                    for(uint i=0;i<subclassPerkSet.size(); i++)
                    {
                        ...
                        if (i == 0)
                        {
                            ...
                            DO_LOG4("Setting core perk slot %d to %f", i, newPerk);
                            *perk = RValue(newPerk);
                            continue;
                        }
                        if (i == subclassPerkSet.size() - 1)
                        {
                            ...
                            DO_LOG4("Setting pistol perk slot %d to %f", i, newPerk);
                            *perk = RValue(newPerk);
                            continue;
                        }
                        ...
                        DO_LOG4("Setting other perk slot %d to %f", i, newPerk);
                        *perk = RValue(newPerk);
                    }
                }
            }
    ...
    if (ImGui::BeginMenu("Objects"))
    {
        ...
        if (ImGui::BeginMenu("Spawn"))
        {
            ...
            //         }
                    auto fn = [&] (int32_t objIndex) {
                        ...
                        DO_LOG4("Calling instance_create (%p) for object index %d", &gml_Script_instance_create, objIndex);
        ...
        if (ImGui::BeginMenu("Replace Bullets"))
        {
            ...
            ImGui::NewLine();
            ImGui::Text(*getReplaceBulletIndex() ? DoBuiltin(&gml_object_get_name, *getReplaceBulletIndex()).ToCString() : "None");
            auto fn = [&] (int32_t objIndex) {
    ...
    if (ImGui::BeginMenu("Rooms"))
    {
        ...
        if (ImGui::BeginMenu("Goto Room"))
        {
            ...
            if (teleporting)
            {
                ...
                (*objRoomCtrl->InternalReadYYVar(VAR_HASH(roomTransitionTarget))) = Organik::Rooms::AssetIndexes[roomID];
                ...
                auto rBuffIndex = buffer.ToInt32();  // buffer of packet data
                RValue ret_DoesntMatter = RValue(-5);
                short packetID = 37;
                short roomID = (short)Organik::Rooms::AssetIndexes[roomID];
                ...
                RValue ret = RValue(0);
                RValue rvrBuffIndex = rBuffIndex;
                RValue rrReliable =  rReliable;
                std::vector<RValue*> args =
                {
                    &rvrBuffIndex, // the buffer
                    & rrReliable   // u can also just lie
                };
