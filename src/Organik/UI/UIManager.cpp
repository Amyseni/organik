...
namespace Organik 
{
    ...
    void UIManager::DrawEvent()
    {
        ...
        {
            static auto cbHash = typeid(ChatBox).hash_code();
...
{
    DO_LOG4("Initializing ImGui... (d3d pointers): %p, %p ", d3d11device, d3d11context);
    if (!d3d11device || !d3d11context)
    {
        DO_LOG0S("D3D11 device or context is not set.");
        return false;
    }
    if (!(Organik::UIManager::isImGuiInitialized()))
    {
        ...
        // t.detach();
        DO_LOG4S("ImGui setup context grabbed successfully");
...
HOOK_GLOBAL(GR_D3D_Finish_Frame, (bool present) -> bool)
{
    ...
    if(ImGui::GetCurrentContext() == nullptr)
    {
        ...
        if (frameCount < 10)
        {
            ...
            frameCount++;
            DO_LOG5("Waiting for ImGui to be initialized. Frame count: %d", frameCount);
            return super(present);
        }
        DO_LOG5S("Waiting for context to be ready...");
        DO_LOG5S("Checking window handle.");
        static std::shared_mutex installMutex;
        std::unique_lock<std::shared_mutex> lock(installMutex); Organik::UIManager::g_hWnd = GetWindowHandle();
        if (Organik::UIManager::g_hWnd == NULL)
        {
            DO_LOG5S("Window handle is NULL, waiting for it to be set.");
            return super(present);
        }
        
        DO_LOG4S("Window is not minimized.");
        ...
        DO_LOG1("D3D11 RValues: 0x%p, 0x%p", (d3d11device), (d3d11context));
        if (!d3d11device || !d3d11context)
        {
            DO_LOG1("%s:%d --- %s", __FILE__, __LINE__, __FUNCTION__);
    ...
    if(ImGui::GetCurrentContext() == nullptr)
    {
        DO_LOG0S("ImGui context is null.");
        return super(present);
    }
    ...
    bool ret = super(present);
    DO_LOG5("D3D frame finished. returned %s", (ret ? "true" : "false"));
    return ret;
}
...
_DEFINE_GLOBAL_HOOK1(__LINE__, WndProc, 0, (HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) -> LRESULT, __stdcall)
{
    DO_LOG5("WndProc called with (%p, %d, %lx, %lx)", hWnd, msg, wParam, lParam);
    ...
    switch (msg) {
        ...
        case WM_MOUSEMOVE:
            ...
            {
                ImGui::GetIO().ConfigFlags &= (~ImGuiConfigFlags_NoMouseCursorChange); // keep every bit except bit 6
                imgui_wants_input = true;
                ImGui::GetIO().MouseDrawCursor = true;
            }
            else 
            {
                ImGui::GetIO().ConfigFlags |= ImGuiConfigFlags_NoMouseCursorChange; // turn on bit 6
