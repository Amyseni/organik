    ...
    class ChatBox : public UIElement
    {
        ...
        public:
            ...
            void AddTab(T* tab)
            {
                ...
                std::unique_lock<std::shared_mutex> lock(tabsListLock);
            ...
            template <typename T>
            requires std::is_base_of_v<ChatTab, T>
            _NODISCARD_LOCK std::pair<std::shared_lock<std::shared_mutex>, T*> GetTab(int32_t hash = 0)
            {
                ...
                if (it != tabs.end()) {
                    ...
                    if (tab) {
                        return
                        {
                            std::move(tab->GetReadLock( )),
                            tab
                        };
                    }
                }
                
                static std::shared_mutex noReturn;
                
                return std::make_pair(std::shared_lock<std::shared_mutex>(noReturn), (T*) nullptr);
            }
